...
public class SearchModsScreen extends AddFromLinkScreen {
	...
	public SearchModsScreen(String minecraftVersion, ModLoader modLoader, ConfigScreen configScreen) {
		...
		modsList = new DashboardList((data, index) -> {
			...
			switch (modId.modProvider) {
				case CURSE_FORGE:
					Util.getOperatingSystem().open(String.format("https://minecraft.curseforge.com/projects/%s", modId.modId));
					break;
				case MODRINTH:
					Util.getOperatingSystem().open(String.format("https://modrinth.com/mod/%s", modId.modId));
					break;
			}
		}), "+", 3);
	}
	...
	protected void renderAdditional(MatrixStack matrices) {
		modsList.render(matrices, textRenderer);
	}
